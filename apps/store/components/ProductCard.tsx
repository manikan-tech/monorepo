  ...
  return (
    <Link href={`/store/${product.slug || product.id}`} className="group flex flex-col gap-4 cursor-pointer">
      <div className="relative aspect-[3/4] w-full overflow-hidden rounded-2xl bg-[#F3F7F7] shadow-soft transition-all duration-500 group-hover:shadow-lift group-hover:-translate-y-2">
        ...
        {/* Wishlist Button */}
        <button 
          className="absolute top-4 right-4 p-2.5 rounded-full bg-white/90 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-white hover:scale-110 shadow-sm text-forest-900 hover:text-gold-500 z-10"
          onClick={(e) => {
